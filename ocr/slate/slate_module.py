...
class SLATE_Module(nn.Module):
    ...
    def forward(self, obs: Tensor, with_attns, with_masks, previous_slots=None) -> Tensor:
        ...
        if self._use_cnn_feat:
            ...
            if with_attns or with_masks:
                slots, attns = self._get_slots(obs, with_attns=True, prev_slots=previous_slots)
                ...
                return self._get_slots(obs, prev_slots=previous_slots)
    ...
    def visualize(self, obs: Tensor, slots, with_attns=False, normalize_slots=False):
        ...
        emb = emb.permute(0, 2, 3, 1).flatten(start_dim=1, end_dim=2)
        # apply slot attention
        attns = self._slotattn.visualize(emb, slots, normalize_slots=normalize_slots)
        attns = attns.transpose(-1, -2).reshape(
            obs.shape[0], self._num_slots, 1, self._obs_size, self._obs_size
        )
        if with_attns:
