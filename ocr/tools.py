...
class SlotExtractor(ABC):
    ...
    def get_slots(self, images, previous_slots, to_numpy=False):
        one_image = len(images.shape) == 3
        if one_image:
            batch_images = images[None, ...]
        else:
            batch_images = images

        if previous_slots is not None and one_image:
            batch_prev_slots = previous_slots[None, ...]
        else:
            batch_prev_slots = previous_slots

        batch_images = batch_images.to(torch.float32) / 255.0
